...
[role="lead"]
Puede utilizar la página SnapCenter *Agregar host* para agregar hosts de Windows. El plugin para Exchange se instala automáticamente en el host especificado. Este es el método recomendado para la instalación de plugins. Puede añadir un host e instalar un plugin para un host individual o para un clúster.
...
Puede añadir un host e instalar paquetes de plugins para un host individual o para un clúster. Si instala plugins en un clúster (Exchange DAG), se instalarán en todos los nodos del clúster aunque algunos nodos no tengan base de datos en las LUN de NetApp.
...
También puede añadir el clúster DAG sin IP mediante la dirección IP o el FQDN de uno de los nodos del clúster DAG.
